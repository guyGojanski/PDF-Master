...
export function MergeTab() {
  ...
  const [rowStatus, setRowStatus] = useState<Record<string, 'green' | 'red' | undefined>>({})
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      const res = await axios.post('http://127.0.0.1:8000/check-password', formData)
      if (res.data.ok) {
        ...
        if (lockedFiles.length <= 1) { setStatus({type: '', message: ''}) }
      } else {
  ...
  const canMerge = files.length > 0 && files.length <= 20 && files.every(f => {
  ...
  const handleMergeClick = async () => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/merge', formData, {
        responseType: 'blob'
      })
  ...
  return (
    <Card>
      <CardHeader>
         ...
         <CardDescription>Select multiple files to merge them into one.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6 relative">
        ...
        >
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm font-medium text-slate-600">Drag files here or click to select</p>
          </div>
          {isDragging && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 transition-all">
              <span className="text-3xl font-bold text-white drop-shadow-lg">Drop here</span>
            </div>
          )}
        </div>
        {files.length > 0 && (
          <div className="space-y-2">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-md border transition-colors ${
                      rowColor === 'green' ? 'bg-green-50 border-green-300' :
                      rowColor === 'red' ? 'bg-red-50 border-red-300' :
                      isError ? 'bg-red-50 border-red-300' : 'bg-slate-100 border-slate-200'
                    }`}
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      <FileText className={`w-4 h-4 flex-shrink-0 ${isError || rowColor === 'red' ? "text-red-500" : rowColor === 'green' ? "text-green-600" : "text-slate-500"}`} />
                      <span className={`text-sm truncate ${isError || rowColor === 'red' ? "text-red-700 font-medium" : rowColor === 'green' ? "text-green-700 font-medium" : ""}`}>
                        {file.name}
                      </span>
                      {(hasPassword || isError) && (
                        <Button
                          ...
                          className={`h-6 w-6 p-0 rounded-full ${
                            isError || rowColor === 'red' ? "text-red-500 hover:text-red-700 hover:bg-red-100 animate-pulse" :
                            isVerified || rowColor === 'green' ? "text-green-600 hover:text-green-700 hover:bg-green-100" : "text-yellow-500 hover:text-yellow-600 hover:bg-yellow-100"
                          }`}
                          onClick={() => openUnlockModal(file.name)}
                          title={isError ? "Click to enter password" : isVerified ? "Password verified" : "Password saved"}
                        >
                          {isVerified || rowColor === 'green' ? <CheckCircle2 className="w-4 h-4" /> : <LockKeyhole className="w-4 h-4" />}
                        </Button>
                      )}
                    </div>
            ...
            <Button onClick={handleMergeClick} disabled={isUploading || !canMerge} className="w-full mt-4 bg-blue-600 hover:bg-blue-700">
               {isUploading ? "Processing..." : "Merge files now 🚀"}
            </Button>
          </div>
        )}
        {status.message && (
          <div className={`text-center p-4 rounded-md font-medium border flex items-center justify-center gap-2 ${
        ...
        {errorPopup && (
          <div className="fixed top-8 left-1/2 -translate-x-1/2 bg-red-600 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center gap-2">
            ...
            {errorPopup}
            <button className="ml-4 text-white text-lg" onClick={() => setErrorPopup(null)}>&times;</button>
          </div>
        )}
